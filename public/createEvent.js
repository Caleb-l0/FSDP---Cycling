...
async function createEvent() {
  ...
  try {
    ...
    // Try to get organization ID and send emails (optional, don't block on errors)
    try {
      const OrganID = await getOrganisationID();
      if (OrganID) {
        const AllUserEmail = await getUserEmail(OrganID);
      }
    } catch (emailError) {
